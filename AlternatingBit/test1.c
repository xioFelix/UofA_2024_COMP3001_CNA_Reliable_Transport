...
#define RTT \
  15.0 /* round trip time.  MUST BE SET TO 15.0 when submitting assignment */
...
static struct pkt
    buffer[WINDOWSIZE]; /* array for storing packets waiting for ACK */
static int windowfirst,
    windowlast; /* array indexes of the first/last packet awaiting ACK */
...
void A_output(struct msg message) {
  ...
  if (windowcount < WINDOWSIZE) {
    ...
    tolayer3(A, sendpkt);
    /**** 1. FILL IN CODE There's something else A needs to do when it sends a
     * packet. *****/
    starttimer(A, RTT);  // Start timer for retransmission

    A_nextseqnum = (A_nextseqnum + 1) % 2; /* we only have seqnum 0 and 1 */
    // printf("---------------------------A_nextseqnum in A_output is %d\n",
    // A_nextseqnum);  // Debug
  }
...
void A_input(struct pkt packet) {
  ...
  if (!IsCorrupted(packet)) {
    ...
    total_ACKs_received++;
    // printf("---------------------------A_nextseqnum in A_input is %d\n",
    // A_nextseqnum);  // Debug
    /* check if new ACK or duplicate */
    if (packet.acknum == (A_nextseqnum + 1) % 2) { /**** 2. FILL IN CODE
     replace TRUE with test whether this is a new ACK ***/
      /* packet is a new ACK */
      if (TRACE > 0)
...
void B_input(struct pkt packet) {
  ...
  if ((!IsCorrupted(packet)) && (packet.seqnum == expectedseqnum)) {
    ...
    sendpkt.acknum =
        (expectedseqnum + 1) % 2;  // Toggle the sequence number for ACK
  }
